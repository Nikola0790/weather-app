...
const TodaysWeather = ({ screen }) => {
  ...
  if (data.length === 0) {
    return <p data-testid="test-3">Loading</p>;
  }
  ...
  return (
    <div className="box_todays_weather">
      <p>Todays weather</p>
      <div className="order_box" data-testid="test-2">
        {listOfData.map((item, index) => {
          if (index < 7) {
            let icon = item.weather[0].icon;
            let temp = Math.round(item.main.temp);
            ...
            return (
              <div
                className="small_box_weather"
                key={index}
                data-testid="test-ancestor"
              >
                <p>{time} h</p>
                <img
                  src={`http://openweathermap.org/img/wn/${icon}@2x.png`}
                  alt=""
                  data-testid="test-descendant"
                />
                <p>
                  {temp} <span>&#176;</span>
                </p>
